...
def alarmOn():
    if sendCommand("server.pi", "python3 /home/pi/Alarmsysteem/server/alarmTrigger.py") == False:
        ...
        sendCommand("server.pi", "python3 /home/pi/Alarmsysteem/server/alarmTrigger.py")
        GPIO.output(rood, GPIO.HIGH)
        GPIO.output(geel, GPIO.LOW)
        GPIO.output(groen, GPIO.LOW)
...
def alarmOff(escape):
    ...
    GPIO.output(groen, GPIO.HIGH)
    if escape == False:
        if sendCommand("server.pi", "python3 /home/pi/Alarmsysteem/server/alarmClear.py") == False:
